...
function initializeButtons(slug, inputElement, sidePanel, projectData) {
    ...
    if (blogToggle) {
        const hasBlogTag = Array.isArray(initialTags) && initialTags.includes('sys:public');
    ...
    if (actionButton) {
        if (!slug) {
            ...
            actionButton.addEventListener('click', () => {
                ...
                const hasBlogTag = project_tags.includes('sys:public');

                if (isBlog && !hasBlogTag) {
                    project_tags.push('sys:blog');
                } else if (!isBlog && hasBlogTag) {
                    project_tags = project_tags.filter(tag => tag !== 'sys:public');
                }
