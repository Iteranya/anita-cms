...
def serve_blog_index(page_service: PageService = Depends(get_page_service)):
    ...
    page = page_service.get_first_page_by_tag('blog-home')
    if not page:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Blog index page not found.")
...
def api_list_blog_pages(page_service: PageService = Depends(get_page_service)):
    """
    API endpoint to get a list of all public pages tagged with 'blog'.
