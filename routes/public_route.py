...
from data.db import get_page, list_pages
...
async def serve_custom_page(request: Request):
    ...
    home_page = next(
        (page for page in pages if page.tags and "home" in page.tags), None
    )
...
async def home(request: Request):
    ...
    blog_home = next(
        (page for page in pages if page.tags and "blog-home" in page.tags), None
    )
    ...
    return templates.TemplateResponse(
        "blog.html", {"request": request, "pages": blog_pages}
    )
...
async def render_site(slug: str):
    ...
    pages = list_pages()
    template = next(
        (page for page in pages if page.tags and "blog-template" in page.tags), None
    )
    if not page:
